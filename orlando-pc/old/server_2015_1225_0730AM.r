...
shinyServer(function(input, output) {
    ...
    output$map<-renderLeaflet({
        ...
        calls <- if (input$year == "2014") read.csv("calls_2014_munged.csv") else if (input$year == "2013") read.csv("calls_2013_munged.csv")
        ...
        relevant_data <- data_filter(calls, location, crimes_set, crimes_days, crimes_periods)
    ...
    output$barplots <- renderPlot({
        ...
        crime_barplot <- ggplot(data = relevant_data) + ggtitle(paste("Number of police calls in", input$year, "faceted by", input$plots_facets[1])) # basic plot + title
        # Arrange variables in the barplot according to user-selected facets
        crime_barplot <- if (input$plots_facets[1] == "crime category") crime_barplot + geom_bar( aes(x = periods, fill = days), position="dodge") + facet_grid( categories~. ) 
                         else if (input$plots_facets[1] == "day of week") crime_barplot + geom_bar( aes(x = periods, fill = categories), position="dodge") + facet_grid( days~. ) 
