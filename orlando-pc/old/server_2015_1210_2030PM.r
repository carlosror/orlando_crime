...
shinyServer(function(input, output) {
    ...
    output$map<-renderLeaflet({
        ...
        relevant_data <- subset(calls, (dist_equi(location[1], location[2], longitude, latitude) < input$radius) & categories %in% crimes_set) 
    ...
    output$mytable <- renderDataTable({
        ...
        crimes_set <- input$crimes
        relevant_data <- subset(calls, (dist_equi(location[1], location[2], longitude, latitude) < input$radius) & categories %in% crimes_set) 
        relevant_data_markers <- relevant_data[c("event_time", "reason", "categories")]
