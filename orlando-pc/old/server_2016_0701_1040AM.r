...
shinyServer(function(input, output, session) {
    ...
    filter_data <- reactive({
        input$goButton
        calls <- isolate(if (input$year == "2014") read.csv("calls_2014_munged.csv") else if (input$year == "2013") read.csv("calls_2013_munged.csv"))
    ...
    output$references <- renderUI({includeHTML("template.html")})
