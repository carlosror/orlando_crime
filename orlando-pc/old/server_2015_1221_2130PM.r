...
shinyServer(function(input, output) {
    ...
    output$barplots <- renderPlot({
        ...
        qplot(x=days, data=relevant_data, geom = "bar", fill=categories, facets = periods~., position = "dodge", ylab = "Police calls")
        
        # crime_barplot <- ggplot(data = relevant_data, aes(days, fill=categories)) + geom_bar( position="dodge") 
        # crime_barplot + facet_wrap( periods~. )
    })
    # Summary table
